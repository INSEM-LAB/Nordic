...
#if 1	//jameslee
const struct uart_ipc_config *config_1;
struct uart_ipc_data *data_1;

int ipc_uart_uninit(void)
{
	int err;
	
	err = ipc_service_deregister_endpoint(&data_1->ept);
	if (err) {
		LOG_ERR("Failed to deregister endpoint (err %d)", err);
		return err;
	}
	
	err = ipc_service_close_instance(config_1->ipc);
	{
		LOG_ERR("Failed to close endpoint (err %d)", err);
		return err;
	}
}
#endif	//jameslee
